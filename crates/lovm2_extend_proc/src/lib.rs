...
pub fn lv2_module_init(_args: TokenStream) -> TokenStream {
    ...
    let result = quote! {
        #[doc(hidden)]
        #[no_mangle]
        pub extern fn #initfn(lib: std::rc::Rc<Library>, slots: &mut std::collections::HashMap<lovm2_extend::prelude::LV2Variable, lovm2_extend::prelude::LV2CallableRef>) {
            #(
                let slot = lovm2_extend::prelude::LV2SharedObjectSlot::new(lib.clone(), #names).expect("name not found");
                slots.insert(
                    lovm2_extend::prelude::LV2Variable::from(#names),
                    std::rc::Rc::new(slot)
                );
            )*
        }
