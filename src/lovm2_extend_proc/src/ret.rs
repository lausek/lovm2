    ...
    pub fn generate(&self) -> impl quote::ToTokens {
        match self {
            Self::Ident(_) | Self::Result(_) => {
                ...
                quote! {
                    #raise_error
                    let val: lovm2_extend::prelude::LV2Value = #ident.into();
            ...
            Self::Maybe(_) => {
                ...
                quote! {
                    if let Some(val) = #ident {
                        let val: lovm2_extend::prelude::LV2Value = val.into();
                        vm.context_mut().push_value(val);
                    } else {
                        vm.context_mut().push_value(lovm2_extend::prelude::LV2Value::Nil);
                    }
                    Ok(())
                }
            }
